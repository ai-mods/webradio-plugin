...
function formatStations(stations: RadioStation[]): string {
  if (stations.length === 0) return 'No stations found.';
  return stations
    .map(
      (s, i) =>
        `${i + 1}. **${s.name}** — ${s.country || 'Unknown'}` +
...
server.registerTool(
  'radio_search',
  {
    ...
    description:
      'Search for internet radio stations by name, country, or genre tag. ' +
  ...
  async ({ query, country, tag, limit }) => {
    ...
    try {
      ...
      return { content: [{ type: 'text' as const, text: formatStations(stations) }] };
    } catch (error) {
      return {
        content: [
          {
...
server.registerTool(
  'radio_get_station',
  {
    ...
    description:
      'Get details of a specific radio station by its UUID, including stream URL. ' +
  ...
  async ({ station_uuid }) => {
    try {
      ...
      if (!station) {
        return {
          content: [{ type: 'text' as const, text: `Station not found: ${station_uuid}` }],
        };
      }
      const info = [
        `Name: ${station.name}`,
...
server.registerTool(
  'radio_top_stations',
  {
    ...
    description:
      'Get the most popular radio stations worldwide, sorted by click count. ' +
      'Always present stream URLs as clickable markdown links so users can listen directly.',
    inputSchema: {
      limit: z
        .number()
