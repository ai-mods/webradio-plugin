 ...
 */
export async function searchStations(query, country, tag, limit = 10) {
 ...
 */
export async function getTopStations(limit = 10) {
    const res = await fetch(`${RADIO_API}/json/stations/topclick?limit=${Math.min(limit, 30)}&hidebroken=true`, { headers: { 'User-Agent': USER_AGENT } });
    if (!res.ok)
        throw new Error(`Radio Browser API error: ${res.status}`);
    return (await res.json());
}
